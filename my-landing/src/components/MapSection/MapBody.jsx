  ...
  return (
    <div className="position-relative min-vh-80 d-flex flex-column align-items-center overflow-hidden">
            ...
            {({ geographies }) =>
              geographies.map((geo) => (
                <Geography
                  ...
                  geography={geo}
                  style={{
                    default: { fill: 'rgb(17,53,65)', stroke: 'white', strokeWidth: 1.5 },
                    hover: { fill: 'rgb(154, 167, 172)', stroke: 'white', strokeWidth: 1.5, cursor: 'pointer' },
                    pressed: { fill: 'rgb(17,53,65)', stroke: 'white', strokeWidth: 1.5 },
                  }}
                  onMouseEnter={(e) => setTooltip({ content: geo.properties.name, x: e.clientX, y: e.clientY, visible: true })}
                  onMouseMove={(e) => setTooltip(prev => ({ ...prev, x: e.clientX, y: e.clientY }))}
                  onMouseLeave={() => setTooltip(prev => ({ ...prev, visible: false }))}
                  onClick={() => window.location.href = `https://afrijour.web.app/?query=${encodeURIComponent(geo.properties.name)}`}
      ...
      {tooltip.visible && (
        <div
          className="position-fixed text-warning px-3 py-2 rounded shadow border border-warning"
          style={{ backgroundColor: 'rgb(17,53,65)', left: tooltip.x + 10, top: tooltip.y - 40, transform: 'translateX(-50%)', pointerEvents: 'none', zIndex: 50 }}
        >
          {tooltip.content}
        </div>
      )}
