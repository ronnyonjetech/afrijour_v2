...
// const normalizedCountryCoordinates = Object.fromEntries(
...
//   const [mapSize, setMapSize] = useState(445);
//   const [maxJournals, setMaxJournals] = useState(1); // Track highest journal count
...
//     fetch("http://127.0.0.1:8000/journal_api/api/journals/country-count/")
//       .then((response) => response.json())
//       .then((data) => {
//         setJournalData(data);
//         setMaxJournals(Math.max(...data.map((entry) => entry.journal_count), 1));
//       })
...
//   // Function to determine circle size based on journal count
...
//   return (
//     <div className="d-flex justify-content-center align-items-center w-100 position-relative" style={{ overflow: "visible", height: "auto" }}>
//       <div className="position-relative w-100" style={{ maxWidth: "800px", overflow: "visible" }}>
//         <ComposableMap
...
//                   geography={geo}
//                   style={{
//                     default: { fill: "#113541", stroke: "#fff", strokeWidth: 1.2 },
//                     hover: { fill: "#255E66", stroke: "#fff", strokeWidth: 1.5, cursor: "pointer" },
...
//           {/* Journal Data Markers */}
//           {journalData
//             .filter(({ country }) => normalizedCountryCoordinates[country?.trim().toLowerCase()])
//             .map(({ country, journal_count }) => {
//               const coordinates = normalizedCountryCoordinates[country.trim().toLowerCase()];
//               return (
//                 <Marker
//                   key={country}
...
//                   }
//                 >
//                   <circle r={getDotSize(journal_count)} fill="red" stroke="white" strokeWidth={1.2} style={{ cursor: "pointer" }} />
//                 </Marker>
//               );
//             })}
//         </ComposableMap>
...
//                   (entry) => entry?.country?.trim().toLowerCase() === countryName.trim().toLowerCase()
//                 );
...
//                     onMouseLeave={() => setTooltip((prev) => ({ ...prev, visible: false }))}
//                     onClick={() =>
//                       window.location.href = `https://afrijour.web.app/?query=${countryName.replace(/\s+/g, "").toLowerCase()}`
//                     }
//                   />
//                 );
//               })
